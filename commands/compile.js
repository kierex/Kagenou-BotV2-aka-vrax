...
module.exports = {
    ...
    execute: async (api, event, args, commands, prefix, admins, appState, sendMessage) => {
        ...
        try {
            ...
            // Improved language detection using regex for different programming languages
            ...
            if (language === 'unsupported') {
                sendMessage(api, { threadID, message: '❌ Unsupported language. Please provide a valid code snippet.' });
                return;
            }

            // Send the code to the compilation API
            const { data } = await axios.post('https://apiv3-2l3o.onrender.com/compile', {
                language,
                code,
                input: '' // Allow the input to be empty if not provided
            });

            // Send back the output formatted as code
            sendMessage(api, { threadID, message: `\`\`\`\n${data.output}\n\`\`\`` });
        } catch (error) {
            // Handle errors in API call and format the error message as code
